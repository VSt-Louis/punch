...
import { InTime, statusFromActions } from './statusFromActions'
...
const inTimeHours = (inTime: InTime) => (inTime.end - inTime.start) / MS_IN_AN_HOUR

export default (actions: Action[], opts: StatusOptions) => {
...
${opts.period || opts.day || opts.week || opts.month ? report(status.inTimes, opts) : ''}
`
}
  ...
  inTimes.forEach(inTime => {
    if (blocks.month) {
      ...
      if (!isInMonth(blocks.month.start, new Date(inTime.start))) {
        if (![opts.week, opts.day, opts.period].some(i => i)) {
    ...
    if (blocks.week) {
      ...
      if (!isInWeek(blocks.week.start.getTime(), inTime.start)) {
        if (![opts.day, opts.period].some(i => i)) {
    ...
    if (opts.period) _report += `${readableDate(new Date(inTime.start))} hours: ${inTimeHours(inTime) + '\n'}`
  })
  return _report
}
