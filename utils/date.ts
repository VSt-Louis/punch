...
export const readableDate = (d: Date): string => dayjs(d).format('YYYY MMMM DD')
export const invoiceDate = (d: Date): string => dayjs(d).format('YYMMDD')
export const readableMonth = (d: Date): string => dayjs(d).format('MMMM')
export const readableWeek = (d: Date): string => `Week ${weekNumber(d)}`
export const readableDay = (d: Date): string => dayjs(d).format('dddd DD')
...
export const weekStartFormatted = (d: Date): string => readableDate(d) + readableWeek(d)
export const weekStartOfDate = (d: Date): Date =>
    new Date(
        new Date(d.getFullYear(), d.getMonth(), d.getDate()).getTime() -
            MS_IN_A_DAY * ((d.getDay() - WEEK_START + 7) % 7) //days into the current week
    )
...
export const isInMonth = (monthStartDate: Date, inTime: Segment) => {
    const date = new Date(inTime.start)
    return date.getFullYear() === monthStartDate.getFullYear() && date.getMonth() === monthStartDate.getMonth()
}
